...
const customerProductReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case CREATE_PRODUCT_LIST_REQUEST:
      return {
        ...state,
        loading: true,
        error: null,
    ...
    case CREATE_PRODUCT_LIST_SUCCESS:
      return {
        ...state,
        loading: false,
        createProductStatus: action.payload,
        products: [...state.products, ...action.payload.newProducts], // Assuming `newProducts` is returned in the payload
      };
